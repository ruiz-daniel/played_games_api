...
namespace DRG_Api.Controllers
{
    ...
    public class Top10CharactersController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<ActionResult<IEnumerable<Top10Character>>>
        GetTop10Characters()
        {
        ...
        [HttpGet("{name}")]
        public async Task<ActionResult<IEnumerable<Top10Character>>>
        GetTop10CharactersBy(string name)
        {
            var top10name =
                await _repositories
                    .Top10Names
                    .FindOneBy(top10name => top10name.name.Contains(name));
            var top10 =
                await _repositories
                    .Top10Characters
                    .FindBy(character => character.top10nameid == top10name.id);

            return top10;
        }

        [HttpPost("{top10name}")]
        public async Task<ActionResult<Top10Character>>
        PostTop10Characters(Top10Character character, string top10name)
        {
            var top10nameObject =
                await _repositories
                    .Top10Names
            ...
            return CreatedAtAction(nameof(GetTop10Characters),
            new { id = character.id },
            character);
        }
        ...
        PutTop10Characters(int id, Top10Character character)
        {
            if (id != character.id)
            {
                return BadRequest();
